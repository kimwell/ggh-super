<template>
  <div class="panel">
    <Card :bordered="false" class="card">
      <p slot="title">保证金等级管理</p>
      <Button slot="extra" size="small" type="success" @click="openModel(false)">新增保证金等级</Button>
      <div class="card-contnet">
        <div class="table-contnet">
          <Row class-name="head">
            <Col class-name="col" span="6">保证金等级名称</Col>
    ...
    <Modal v-model="show" :title="isEdit?`编辑保证金等级`:`添加保证金等级`" :closable="false" :mask-closable="false">
...
<script>
  export default {
    ...
    methods: {
      getList() {
        this.$http.post(this.api.findMarginLevelPage, this.pageData).then(res => {
          if (res.code === 1000) {
      ...
      deleteItem(data) {
        this.$Modal.confirm({
          ...
          onOk: () => {
            ...
            this.$http.post(this.api.removeMarginLevel, params).then(res => {
              if (res.code === 1000) {
      ...
      handleSubmit() {
        this.$refs[this.ref].validate((valid) => {
          if (valid) {
            ...
            this.$http.post(this.api.saveAndUpdateMarginLevel, params).then(res => {
              if (res.code === 1000) {
                this.getList();
