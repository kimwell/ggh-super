  ...
  <div class="panel">
    <Card :bordered="false" class="card">
      ...
      <div class="card-contnet">
        ...
        <Page class="page-count" size="small" show-total :total="totalCount" :page-size="apiData.pageSize" @on-change="changePage"></Page>
      </div>
    </Card>
  </div>
</template>
...
<script>
  export default {
    ...
    watch: {
      filterData: {
        handler: _.debounce(function(val, oldVal) {
          // 是否是翻页操作
          if (val.currentPage == oldVal.currentPage) this.apiData.currentPage = 1;
          this.getList();
        }, 200),
        deep: true
      }
    ...
    methods: {
      ...
      getList() {
        this.$http
          .post(this.api.findNotBuserInfoPage, this.filterData)
          .then(res => {
            if (res.code === 1000) {
      ...
      goRouter(data) {
        this.$router.push(
          "examineDetail/" + data.companyId + "/" + data.userId + "/" + data.pass
        );
      }
    },
    created() {
